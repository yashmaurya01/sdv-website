...
from torch.nn import (
    BatchNorm2d, Conv2d, ConvTranspose2d, LeakyReLU, Module, ReLU, Sequential, Sigmoid, Tanh, init, BCELoss, Dropout, Flatten)
...
class TableGAN(LegacySingleTableBaseline):
    ...
    def fit(self, data, categorical_columns=tuple(), ordinal_columns=tuple()):
        ...
        self.transformer.fit(pd.DataFrame(data, columns=self.columns), categorical_columns, ordinal_columns)
...
#                 print(real.shape, len(data))
#                 print("REA", real[0].max())
#                 print(fake[0])
                y_real = discriminator(real)
                y_fake = discriminator(fake)
#                 real_loss = criterion(y_real, torch.full(y_real.shape, 1.0).to(self.device)) # label smoothing
#                 fake_loss = criterion(y_fake, torch.zeros_like(y_fake))
#                 total_loss = (real_loss + fake_loss) / 2
                loss_d = (
                    -(torch.log(y_real + 1e-4).mean()) - (torch.log(1. - y_fake + 1e-4).mean()))
                total_loss = loss_d
#                 print(y_real.min(), y_fake.min())
                total_loss.backward()
                optimizerD.step()
...
#                 loss_g = criterion(y_fake, torch.ones_like(y_fake))
                ...
                if((id_ + 1) % 50 == 0):
                    print("epoch", i + 1, "step", id_ + 1, total_loss, loss_g, loss_c)
